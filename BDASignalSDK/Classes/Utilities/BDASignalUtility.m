...
+ (void)requestSignalWithParams:(NSDictionary *)params {
    ...
    NSString *h20;
    Class statAgent = NSClassFromString(@"QHStatAgent");
    SEL m2Selector = NSSelectorFromString(@"getM2");
    if (statAgent && m2Selector) {
        NSString *m2 = [statAgent performSelector:m2Selector];
        if ([m2 isKindOfClass:[NSString class]]) {
            h20 = m2;
        }
    }
    if (!h20) {
